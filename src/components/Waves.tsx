...
const Waves = ({
  ...
  const boundingRef: any = useRef({ width: 0, height: 0, left: 0, top: 0 });
  const noiseRef: any = useRef(new Noise(Math.random()));
  const linesRef: any = useRef([]);
  const mouseRef: any = useRef({
    x: -10,
    y: 0,
    lx: 0,
    ly: 0,
  ...
  const frameIdRef: any = useRef(null);
